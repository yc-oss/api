...
const fetchAllCompanies = async (): Promise<LaunchedCompany[]> => {
  ...
  const baseUrl =
    `https://${applicationId.toLowerCase()}-dsn.algolia.net/1/indexes/*/queries`;
  ...
  for (const [batch, count] of Object.entries(batches)) {
    ...
    while (fetchedCount < count) {
          ...
          requests: [
            {
              indexName,
              params:
                `facets=%5B%22app_answers%22%2C%22app_video_public%22%2C%22batch%22%2C%22demo_day_video_public%22%2C%22highlight_black%22%2C%22highlight_latinx%22%2C%22highlight_women%22%2C%22industries%22%2C%22isHiring%22%2C%22nonprofit%22%2C%22question_answers%22%2C%22regions%22%2C%22subindustry%22%2C%22tags%22%2C%22top_company%22%5D&hitsPerPage=1000&maxValuesPerFacet=1000&query=&tagFilters=&facetFilters=batch:${
                  encodeURIComponent(
                    batch,
                  )
                }&page=${page}`,
      ...
      const batchHits = batchJson.results?.[0]?.hits as
        | LaunchedCompany[]
        | undefined;
      if (!batchHits) {
        throw new Error(
          `Algolia batch response did not include hits for ${batch}`,
        );
      }
...
const results: CompanyResult[] = companies.map((hit) => {
  ...
  return {
    ...
    api: `https://yc-oss.github.io/api/batches/${
      slugify(
...
for (const { key, slug, name } of companyBooleanFilters) {
...
for (const company of results) {
  await ensureDir(
    `batches/${
      slugify(company.batch ?? "Unspecified", {
        lower: true,
      })
    }`,
  );
...
if (hasChanges) {
  ...
  for (const slug of Object.keys(meta.batches)) {
    text += `| ${meta.batches[slug].name} | ${
      meta.batches[slug].count
  ...
  for (const slug of Object.keys(meta.industries)) {
    text += `| ${meta.industries[slug].name} | ${
      meta.industries[slug].count
    } | https://yc-oss.github.io/api/industries/${slug}.json |\n`;
  }
  text += `</details>\n`;
  ...
  for (const slug of Object.keys(meta.tags)) {
    text += `| ${meta.tags[slug].name} | ${
      meta.tags[slug].count
    } | https://yc-oss.github.io/api/tags/${slug}.json |\n`;
  }
  text += `</details>\n`;
